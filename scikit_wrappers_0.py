...
class LinearClassifier(torch.nn.Module):
    ...
    def forward(self, features):
        ...
        feature = torch.flatten(features, start_dim=1)
...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def __init__(self, compared_length, nb_random_samples, negative_penalty,
                 ...
                 gpu=0, Sim_CSE=False, dropout_p=0.1):
        self.architecture = ''
        self.cuda = cuda
        self.gpu = gpu
        ...
        , Sim_CSE=Sim_CSE, dropout_p=dropout_p)
    ...
    def fit_classifier(self, features, y, print_svm_params=False, C=float('inf')):
        """
        ...
        # SVM classifier pytorch
        self.classifier = sklearn.svm.LinearSVC(random_state=0, tol=1e-5, max_iter=10000)
        if C != float('inf'):
        ...
        self.classifier.dual_coef_ = coef
        if print_svm_params:
        ...
        if train_size // nb_classes < 5 or train_size < 50 or self.penalty is not None:
            ...
            self.classifier = grid_search.best_estimator_
            params = self.classifier.get_params()
            coef = self.classifier.dual_coef_
            intercept = self.classifier.intercept_
            self.classifier.dual_coef_ = coef
            print("svm params:", params, len(coef), len(coef[0]), len(intercept), coef, intercept)
            return self.classifier
        '''

    def fit_encoder(self, X, encoder_weights, y=None, save_memory=False, verbose=False):
        """
        ...
        while i < self.nb_steps:
            ...
            for batch in train_generator:
                ...
                if i%100 == 0:
                    print("i index:", i, self.batch_size)
            epochs += 1
    ...
    def score_feats(self, feats, y, batch_size=50):
        """
        Outputs accuracy of the SVM classifier on the given testing data.
        ...
        @param batch_size Size of batches used for splitting the test data to
               ...
               testing set contains time series of unequal lengths.
        """
        return self.classifier.score(feats, y)
...
class CausalCNNEncoderClassifier(TimeSeriesEncoderClassifier):
    ...
    def __init__(self, compared_length=50, nb_random_samples=10,
                 ...
                 in_channels=1, cuda=False, gpu=0, Sim_CSE=False, dropout_p=0.1):
        super(CausalCNNEncoderClassifier, self).__init__(
            ...
            in_channels, out_channels, cuda, gpu,   Sim_CSE=Sim_CSE, dropout_p=dropout_p
        )
    ...
    def get_params(self, deep=True):
        return {
            ...
            'cuda': self.cuda,
            'gpu': self.gpu,
            'Sim_CSE': self.loss.Sim_CSE,
            'dropout_p': self.loss.dropout_p
        }

    def set_params(self, compared_length, nb_random_samples, negative_penalty,
                   batch_size, nb_steps, lr, penalty, early_stopping,
                   channels, depth, reduced_size, out_channels, kernel_size,
                   in_channels, cuda, gpu, Sim_CSE=False, dropout_p=0.1):
        self.__init__(
            compared_length, nb_random_samples, negative_penalty, batch_size,

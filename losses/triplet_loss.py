...
class TripletLoss(torch.nn.modules.loss._Loss):
    ...
    @param negative_penalty Multiplicative coefficient for the negative sample
           loss.
    """
    def __init__(self, compared_length, nb_random_samples, negative_penalty, Sim_CSE=False, dropout_p=0.1):
    ...
    def forward(self, batch, encoder, train, save_memory=False):
        ...
        if self.Sim_CSE:
            positive_representation = encoder(self.dp(torch.cat(
            [batch[
                j: j + 1, :,
                beginning_batches[j]: beginning_batches[j] + random_length
            ] for j in range(batch_size)]
        ...
        for i in range(self.nb_random_samples):
            ...
            if self.Sim_CSE:
                    negative_representation = encoder( self.dp(
    ...
    def forward(self, batch, encoder, train, save_memory=False):
        ...
        for i in range(self.nb_random_samples):
            ...
            if self.Sim_CSE:
                ...
                negative_representation = torch.cat([encoder(self.dp(
